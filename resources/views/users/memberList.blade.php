    ...
    <x-layout.intro-para>
        <table class="table-auto text-sm">
            ...
            @foreach($members as $member)
                ...
                <tr class="border-t-2">
                    <td>
                        <form method="POST" id="form_{{$member->id}}" action="{{route('membershippurchases.store', ['user'=> $member])}}">
@csrf
                        {{$member->full_name}} / {{$member->postcode}}</td>
                    <td> <x-goodbad :success="$member->membershipIsCurrent()">{{$member->getLatestMembershipEndDate()?->format('Y-m-d') ?? '--'}}</x-goodbad></td>
                    <td class="text-center">
                        <input form="form_{{$member->id}}" type="checkbox" name="can_retain_data" value="1" {!! $member->membershipIsCurrent() ?' disabled="disabled"' : '' !!}{!! $member->can_retain_data ? ' checked="checked"' :'' !!} " />
{{--                        {!! $member->can_retain_data ? 'Yes': 'No <span class="border-4 inline-block w-12">&nbsp;</span>' !!}--}}
                    </td>
                    <td class="text-center">
                        <input form="form_{{$member->id}}" type="checkbox" name="can_email" value="1" {!! $member->membershipIsCurrent() ?' disabled="disabled"' : '' !!} {!! $member->can_email ? 'checked="checked"' :'' !!} " />
{{--                        {!! $member->can_email ? 'Yes': 'No <span class="border-4 inline-block w-12">&nbsp;</span>' !!}--}}
                    </td>
                    <td>
                        @if(!$member->membershipIsCurrent())
                            <input form="form_{{$member->id}}" type="email" name="email" class="text-sm w-64" value="{{$member->email}}" />
{{--                            <span class="border-4 inline-block w-64">&nbsp;</span></td>--}}
                    ...
                    <td>
                        @if(!$member->membershipIsCurrent())
                            <x-select form="form_{{$member->id}}" class="w-40 text-sm" name="membership_type" blankLabel="Membership Type.." :options="\App\Models\Membership::getTypes()" hasBlank="true" :selected="$latestMembershipPurchase?->type"/><br />
                            <x-select form="form_{{$member->id}}" class="w-40 text-sm" name="payment_method" blankLabel="Payment Type.." :options="\App\Models\Payment::getAllPaymentTypes(false)" hasBlank="true"/>
...
{{--                            </select>--}}
                    ...
                    <td>
                        @if(!$member->membershipIsCurrent())
                            <input form="form_{{$member->id}}" type="hidden" name="user_id" value="{{(int)$member->id}}" />
