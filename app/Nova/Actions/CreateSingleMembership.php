...
class CreateSingleMembership extends Action
{
     ...
     * @var string
     */
    public $confirmButtonText = 'Create Single Membership(s)';
    public $name = 'Create Single Membership(s)';
    ...
    {
        $membershipType = Membership::whereAppliesTo(Membership::APPLIES_TO_ENTRANT)->firstOrFail();
        ...
        $models->each(function (User $user) use ($fields, $membershipType) {
            ...
            if (0 === $user->membershipPurchases()->active()->count()) {
                $membershipPurchase = MembershipPurchase::create(
                    [
                        'entrant_id' => $user->entrants->first()?->id,
